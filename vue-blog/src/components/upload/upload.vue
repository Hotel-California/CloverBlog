        ...
        <div class="upload">
          ...
          <div class="right">
            <div class="imgin" @dragleave.prevent.stop="" @drop.prevent.stop="ondrop($event,1)" @dragover.prevent="isImgIn" @dragleave.prevent="isImgOut" ref="selectImg">
              <div v-if="imgList.length<1" class="imgtext">将图片拖入此处
                <svg class="icon" aria-hidden="true">
...
export default {
  ...
  methods: {
    ...
    ondrop (e, num) {
      ...
      if (num === 0) {
        ...
        for (let i = 0; i < e.dataTransfer.files.length; i++) {
          if (!/.(gif|jpg|jpeg|png|gif|jpg|png)$/.test(e.dataTransfer.files[i].name)) {
    ...
    save () {
      if (!(this.fileList.length && this.blog.title && this.blog.category && this.blog.preview)) {

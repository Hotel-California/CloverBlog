...
//        String path = i + ".jpg";
//        ClassPathResource cp = new ClassPathResource("static/background/");
...
//        BufferedImage image = ImageIO.read(inputStream);
    ...
    @ResponseBody
    public Result<User> saveNotice(String id, @RequestBody User user) {
        if(!userService.findId(id)) return Result.fail("用户不存在");
    ...
    public Result<Blog[]> saveBlog(MultipartFile[] mds, MultipartFile[] imgs, @RequestPart("blog") Blog blog ) throws IOException {
        ...
        for(MultipartFile md : mds) {
            ...
            if(!file.exists()) {
                ...
                return Result.fail("相同博客文件在服务器已存在");
            }
        ...
        for(MultipartFile img : imgs) {
            ...
            if(!file.exists()) {
                if (file.mkdirs()) {
                    img.transferTo(file);
                    imgCount++;
                } else throw new RuntimeException("图片创建失败");
            }
        }
...
//    @PostMapping("saveImg")
//    @ResponseBody
//    public Result<User> saveImg(String id, MultipartFile[] imgs) throws IOException {
//        System.out.println(id);
//        for(MultipartFile img : imgs) {
//            File file = new File(userBlogPath, "User"+id+"_at_"+System.currentTimeMillis()+"_".concat(Objects.requireNonNull(img.getOriginalFilename())));
//            if(!file.exists()) {
//                if(file.mkdirs()) {
//                    img.transferTo(file);
//                } else throw new RuntimeException("图片创建失败");
//            } else {
//                return Result.fail("图片已存在");
//            }
//        }
